...
import { allOrders, clearError } from "../actions/orderActions";
...
const AllOrders = () => {
  ...
  const { total, loading, error, orders } = useSelector(
          ...
          return (
            <Card className="mb-3" key={order._id}>
              ...
              <Card.Body>
                ...
                <Card.Text>
                  <Link to={`/admin/users/${order.user}`}>{order.user}</Link>
                </Card.Text>
